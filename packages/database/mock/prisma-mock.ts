// @ts-nocheck
import type { Clock } from '@kanban/clock'
import { LiveClock } from '@kanban/clock'
import type { IdGenerator } from '@kanban/id-generator'
import { UuidV4Generator } from '@kanban/id-generator'
import type { PrismaClient, Prisma } from '@prisma/client'
import dmmfJson from '../prisma/dmmf.json'
  ...
  datamodel.models.forEach((model) => {
    ...
    Object.keys(objs).forEach((fncName) => {
      client[c][fncName] = async (...params) => {
        return await wrapMiddlewares(middlewares, async (params) =>
          objs[fncName](params.args)
        )({
          args: params[0],
          model: model.name,
          action: fncName,
